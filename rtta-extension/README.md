...
The popup asks for the same household code used by RTTA Web and stores it in
`chrome.storage.local`. It sends the code only in the first `AUTH` WebSocket control
frame and never places it in source, URLs, synced storage, or logs. Production CI
injects only the public `wss://api-rtta.dorriss.com/ws/audio` endpoint.
